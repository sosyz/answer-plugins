...
func (e *VectorSearchEngine) sync() {
	...
	go func() {
		...
		if e.Config.EmbeddingLevel == "" || e.Config.EmbeddingLevel == "question" {
			log.Info("qdrant: starting question sync...")
			page = 1
			for {
				log.Infof("qdrant: sync questions page %d", page)
				questions, err := e.syncer.GetQuestionsPage(context.TODO(), page, pageSize)
				if err != nil {
					log.Errorf("qdrant: get questions page failed: %v", err)
					break
				}
				if len(questions) == 0 {
					break
		...
		if e.Config.EmbeddingLevel == "answer" {
			log.Info("qdrant: starting answer sync...")
			page = 1
			for {
				log.Infof("qdrant: sync answers page %d", page)
				answers, err := e.syncer.GetAnswersPage(context.TODO(), page, pageSize)
				if err != nil {
					log.Errorf("qdrant: get answers page failed: %v", err)
					break
				}
				if len(answers) == 0 {
					break
		...
		log.Info("qdrant: sync complete")
	}()
}

func (e *VectorSearchEngine) bulkIndex(ctx context.Context, contents []*plugin.VectorSearchContent) {
	log.Debugf("qdrant: bulkIndex batch size=%d", len(contents))
	success, failed := 0, 0
	for _, c := range contents {
		if err := e.UpdateContent(ctx, c); err != nil {
			log.Warnf("qdrant: index %s failed: %v", c.ObjectID, err)
			failed++
		} else {
			success++
		}
	}
	log.Debugf("qdrant: bulkIndex batch done, success=%d failed=%d", success, failed)
}
